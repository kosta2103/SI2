...
if($tip == "fles_memorija"){
	...
	$kapacitet = $_POST["kapacitet"];
	$povezivanje = $_POST["povezivanje"];
	$sql= "INSERT INTO fles_memorija (Barcode, USB_Type_C, Brzina_citanja_pisanja, Kapacitet, Povezivanje) VALUES ('$barcode', '$format', '$povezivanje', '$kapacitet')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "kablovi"){
...
if($tip == "skener"){
	...
	$rezolucija = $_POST["rezolucija"];
	$adf = $_POST["adf"];
	$sql= "INSERT INTO skener (Barcode, Format, Flatbed, Povezivanje, Rezolucija, ADF) VALUES ('$barcode', '$flatbed', '$povezivanje', '$rezolucija', '$adf')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "slusalice"){
