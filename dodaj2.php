...
if($tip == "kablovi"){
	...
	$sql= "INSERT INTO kablovi (ID, Strana_1, Strana_2, Broj_uticnica, Tip_kablovi, Prekidac, Vrsta) VALUES ('$barcode', '$strana_1', '$strana_2', '$broj_uticnica', '$tip', '$prekidac', '$vrsta')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "mikrofon"){
	$povezivanje = $_POST["povezivanje"];
...
if($tip == "podloga"){
	...
	$sql= "INSERT INTO podloga (ID, Tip_podloga, Materijal) VALUES ('$barcode', '$tip', '$materijal')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "projektor"){
	$povezivanje = $_POST["povezivanje"];
	...
	$rs232 = $_POST["rs232"];
	$vga = $_POST["vga"];
	$sql= "INSERT INTO projektor (ID, Povezivanje, Tip_projektor, Rezolucija, Osvetljenje Wireless, USB, Mreza, HDMI, DVI, RS232, VGA) VALUES ('$barcode', '$povezivanje', '$tip', '$rezolucija', '$osvetljenje', '$wireless', '$usb', '$mreza', '$hdmi', '$dvi', '$rs232', '$vga')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "skener"){
...
if($tip == "slusalice"){
	...
	$gaming = $_POST["gaming"];
	$frekvencijski_raspon = $_POST["frekvencijski_raspon"];
	$sql= "INSERT INTO slusalice (ID, Tip_slusalice, Mikrofon, Zvucni_sistem, Povezivanje, Gaming, Frekvencijski_raspon) VALUES ('$barcode', '$tip', '$mikrofon', '$zvucni_sistem', '$povezivanje', '$gaming', '$frekvencijski_raspon')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "stampac"){
	...
	$mreza = $_POST["mreza"];
	$wireless = $_POST["wireless"];
	$sql= "INSERT INTO stampac (ID, Tip_stampac, Povezivanje, Rezolucija, Brzina_stampe, Bar_kod, Mreza, Wireless) VALUES ('$barcode', '$tip', '$povezivanje', '$rezolucija', '$brzina_stampe', '$bar_kod', '$mreza', '$wireless')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "tastatura"){
	$povezivanje = $_POST["povezivanje"];
	...
	$sql= "INSERT INTO tastatura (ID, Povezivanje, USB_port, Numericki_deo, Tip_tastatura, Tip_tastera, Programabilni_tasteri, RGB_osvetljenje) VALUES ('$barcode', '$povezivanje', '$usb_port', '$numericki_deo', '$tip', '$tip_tastera', '$programabilni_tasteri', '$rgb_osvetljenje')";
	$veza->query($sql) or die($veza->error);
}
if($tip == "zvucnici"){
