...
elseif(isset($_POST["remove"])){
	...
	elseif($vrsta == "Vrsta"){
		...
		$sql= "SELECT * FROM proizvodi WHERE Barcode = '".$naziv."'";
		$result = mysqli_query($veza, $sql);
		while ($row = mysqli_fetch_assoc($result)) {
			$aktuelna_cena = $row["Cena"];
		}
		$cena = $aktuelna_cena/($procenat+1);
		$sql = "UPDATE proizvodi SET Cena = '".$cena."' WHERE Barcode = '".$naziv."'";
		$veza->query($sql) or die($veza->error);
	}
    ...
    if($vazi == 1){
	    ...
	    elseif($vrsta == "Vrsta"){
			...
			$cena = $aktuelna_cena + $aktuelna_cena*$procenat+1;
			$sql = "UPDATE proizvodi SET Cena = '".$cena."' WHERE Barcode = '".$naziv."'";
			$veza->query($sql) or die($veza->error);
		}
    	$vazi = 1;
